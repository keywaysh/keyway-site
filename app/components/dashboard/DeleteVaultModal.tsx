    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md bg-dark border border-white/[0.1] rounded-xl shadow-2xl">
        <div className="flex items-center justify-between p-4 border-b border-white/[0.08]">
          <h2 className="text-lg font-semibold text-red-400">Delete Vault</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-4">
          ...
          {hasSecrets ? (
            <>
                ...
                <div className="flex gap-3">
                  ...
                  <div className="text-sm">
                    ...
                    <p className="text-gray-light mt-1">
                      This will permanently delete the <span className="font-mono text-white">{vault.repo_owner}/{vault.repo_name}</span> vault and all <span className="font-semibold text-white">{vault.secrets_count} secret{vault.secrets_count > 1 ? 's' : ''}</span> it contains.
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="mb-4">
                <label className="block text-sm text-gray-light mb-2">
                  To confirm, type <span className="font-mono text-white">{expectedText}</span> below:
                </label>
                <input
                  type="text"
                  value={confirmText}
                  ...
                  className="w-full px-3 py-2 bg-dark-darker border border-white/[0.1] rounded-lg text-white placeholder-gray-muted focus:outline-none focus:border-red-500/50 font-mono text-sm"
                  autoComplete="off"
                  spellCheck={false}
                />
              </div>
            ...
            <p className="text-gray-light mb-4">
              Are you sure you want to delete the vault <span className="font-mono text-white">{vault.repo_owner}/{vault.repo_name}</span>? This vault is empty.
            </p>
          )}
